...
export async function requestAppointment(input: {
  ...
  if (!phone?.trim() || phone.replace(/\D/g, '').length < 7) return { error: 'Ingresá un teléfono válido.' }
  ...
  const startsAt = new Date(day); startsAt.setHours(h, m, 0, 0)
  if (startsAt < new Date()) return { error: 'Esa fecha ya pasó.' }
  ...
  if (error) return { error: 'No se pudo registrar la solicitud. Intentá de nuevo.' }
  return { success: true }
}
