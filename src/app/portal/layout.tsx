    ...
    <div className="min-h-screen bg-bg flex flex-col">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              ...
              <span className="hidden sm:inline font-medium text-sm text-brand/80 bg-surface/10 px-2 py-0.5 rounded-full">Portal de Paciente</span>
            </div>
            
            <div className="flex items-center gap-4">
              <span className="text-sm font-medium">Hola, {firstName}</span>
              <div className="w-8 h-8 rounded-full bg-surface/20 flex items-center justify-center font-bold">
                {firstName[0]}
              </div>
            </div>
          </div>
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col md:flex-row gap-8">
        ...
        <nav className="md:w-64 flex-shrink-0">
          <div className="bg-surface rounded-2xl shadow-sm border border-border p-4 sticky top-24">
            <ul className="space-y-2">
              {navItems.map((item) => (
                <li key={item.name}>
                  <Link 
                    href={item.href}
                    className="flex items-center gap-3 px-4 py-3 rounded-xl text-muted hover:bg-brand/5 hover:text-brand transition-colors font-medium"
                  >
